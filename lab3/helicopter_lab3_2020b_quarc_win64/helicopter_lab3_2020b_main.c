 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * helicopter_lab3_2020b(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(helicopter_lab3_2020b_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(helicopter_lab3_2020b_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: helicopter_lab3_2020b -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (helicopter_lab3_2020b_DW.HILReadEncoderTimebase_Task, (t_clock)
         helicopter_lab3_2020b_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
